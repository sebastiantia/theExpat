...
const PostEntry = ({ location, setPostLocation, getPosts }) => {
  const cancelButtonRef = useRef(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const newLng = Math.trunc(location.lng * 1000)
  const newLat = Math.trunc(location.lat * 1000)
  const [data, setData] = useState({ longitude: newLng, latitude: newLat, title: null, description: null, image: null, visitDate: null });
